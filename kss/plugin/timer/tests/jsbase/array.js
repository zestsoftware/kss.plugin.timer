...
global.array = new function() {
    ...
    this.contains = function contains(a, element, objectequality) {
        /* see if some value is in a */
        // DEPRECATED!!! use array.indexOf instead
        return (this.indexOf(a, element, !objectequality) > -1);
    };

    this.indexOf = function indexOf(a, element, compareValues) {
    ...
    this.remove = function remove(a, el) {
    ...
    this.removeDoubles = function removeDoubles(a) {
        var ret = [];
        for (var i=0; i < a.length; i++) {
    ...
    this.map = function map(a, func, inplace) {
    ...
    this.filter = function filter(a, func) {
        /* return a copy of 'a' for which filter(el) returns true
        */
        var ret = [];
        for (var i=0; i < a.length; i++) {
            if (func(a[i])) {
    ...
    this.reversed = function reversed(a) {
        var ret = [];
    ...
    this.StopIteration = function StopIteration(message) {
